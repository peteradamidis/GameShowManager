...
- **Enhanced Reschedule Tab:** Added email column and search capability to filter by name, attending with, and email address. Includes duplicate prevention (updates existing entries instead of creating duplicates when contestant is re-rescheduled), reschedule count tracking with visual badges (x2, x3, etc.), decline history storage (previous reasons and dates stored in JSON), rebooked status tracking (shows REBOOKED badge with allocated date when contestant is assigned to a new record day), and a Reschedule History section in the detail dialog showing current entry and all previous decline records.
- **History Page (Consolidated Audit Trail):** A dedicated History tab that consolidates all contestant movement tracking into a single timeline view. Shows rebookings (contestants moved between record days), attendance issues (no-shows and early leavers), and standby attendance (when standbys attended record days). Features include search by contestant name or description, filter by event type, stats cards showing counts for each category, and chronologically sorted event display with timestamps and "by" attribution. Useful for accountability, troubleshooting disputes, and tracking contestant journeys through the system.
...
- **Producer & AP Assignment:** Each record day can have a Producer and Assistant Producer (AP) assigned via dropdowns in the seating chart header. Producer options: Peter Adamidis, Kathleen Reynolds, Maggie Carty. AP options: Lochie Hookway, Felicity Parker-Hill. Selections are saved immediately and persist per record day.
- **Casting Card Version History:** Time-throttled backup/recovery system for casting cards. Automatically saves a version snapshot every 10 minutes during editing (not on every keystroke). The History button in the casting card toolbar opens a dialog showing all saved versions with timestamps. Producers can restore any previous version with one click - a snapshot is taken before restore to enable undo. Stored in `casting_card_versions` table with castingCardId, cardData (JSON), createdAt, and createdBy fields.
- **Animated Welcome Message:** Configurable full-screen animated message that appears once per login session. Settings page allows enabling/disabling the message, customizing the text, selecting animation style (fade/slide/zoom), and toggling confetti effect. Uses framer-motion for animations and canvas-confetti for particle effects. Configuration stored in `system_settings` table. Session tracking via sessionStorage ensures message shows only once per session and resets on logout.

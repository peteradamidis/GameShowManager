...
- **Mobility/Access Notes Indicator (Seating Chart):**
    - Contestants with data in their `mobilityNotes` field (Mobility/Access Notes) display a red plus icon on their seat card.
    - Icon appears next to the contestant's name on the seating chart.
    - Hovering over the icon shows a tooltip with the label "Mobility/Access Notes" and the actual notes text.
    - Makes it easy to identify contestants with mobility or access needs at a glance.

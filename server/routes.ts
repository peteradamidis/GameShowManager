...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Group move - move multiple seat assignments atomically
  app.post("/api/seat-assignments/group-move", async (req, res) => {
    try {
      ...
      // Validate all moves have required fields
      for (const move of moves) {
        if (!move.assignmentId || move.blockNumber === undefined || !move.seatLabel) {
          return res.status(400).json({ error: "Each move must have assignmentId, blockNumber, and seatLabel" });
        }
      }
      
      // Get the first assignment to determine record day
      const firstAssignment = await storage.getSeatAssignmentById(moves[0].assignmentId);
      if (!firstAssignment) {
        return res.status(404).json({ error: "Assignment not found" });
      }
      const recordDayId = firstAssignment.recordDayId;
      
      // Get all assignments for collision detection
      const allAssignments = await storage.getSeatAssignmentsByRecordDay(recordDayId);
      const movingAssignmentIds = new Set(moves.map((m: any) => m.assignmentId));
      
      // Check for collisions at target seats (excluding moving assignments)
      for (const move of moves) {
        const collision = allAssignments.find(
          (a) => !movingAssignmentIds.has(a.id) && 
                 a.blockNumber === move.blockNumber && 
                 a.seatLabel === move.seatLabel
        );
        if (collision) {
          return res.status(400).json({ 
            error: `Seat ${move.blockNumber}-${move.seatLabel} is already occupied`,
            conflictingAssignment: collision
          });
        }
      }
      
      // Execute all moves atomically in a transaction
      const results = await storage.batchUpdateSeatAssignments(moves);
      
      res.json({ message: "Group moved successfully", assignments: results });
    } catch (error: any) {
      res.status(500).json({ error: error.message });
    }
  });

  // Group move with tracking (RX Day Mode)
  app.post("/api/seat-assignments/group-move-tracked", async (req, res) => {
    try {
      const { moves } = req.body;
      ...
      // Execute all moves with tracking atomically in a transaction
      const results = await storage.batchMoveSeatAssignmentsWithTracking(moves);
      
      res.json({ message: "Group moved successfully with tracking", assignments: results });
    } catch (error: any) {
      res.status(500).json({ error: error.message });
    }
  });
